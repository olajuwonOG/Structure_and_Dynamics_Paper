...
#
# Modified by J. Brown 2014-04-02
...
def read_lammpstrj(fname, num_frames=float('inf'), skip_beginning=0, skip_between=0):
    ...
    # 3D array to store boundaries of the box, indexed by frame, x/y/z, then lower/upper
    ...
    # array to map from atom id to molecule id, builds this from the first frame, if available
    id2mol = np.zeros(num_atoms+1, int)
    # array to map from atom id to type, builds this from the first frame, if available
    ...
    for atom in range(num_atoms):
        ...
        if scaled:
            r[frame][my_id][0] = r[frame][my_id][0] * \
                (box_bounds[frame][0][1]-box_bounds[frame]
                 [0][0]) + box_bounds[frame][0][0]
            r[frame][my_id][1] = r[frame][my_id][1] * \
                (box_bounds[frame][1][1]-box_bounds[frame]
                 [1][0]) + box_bounds[frame][1][0]
            r[frame][my_id][2] = r[frame][my_id][2] * \
                (box_bounds[frame][2][1]-box_bounds[frame]
                 [2][0]) + box_bounds[frame][2][0]
    ...
    # this is the actual frame count in the file (not counting the ones skipped in the beginning
    frame_attempt = 0
    while frame < num_frames:
        ...
        if inf_frames:
            ...
            box_bounds = np.concatenate(
                (box_bounds, np.zeros([1, 3, 2], float)))
...
def end2end_autocorr(r, ir, box_bounds, mol2ids):
    ...
    e2e = np.zeros([frames, mols, 3], float)
    e2e_autocorr = np.zeros(frames, float)

    # loop over time
    for t in range(frames):
        box_size = np.array([box_bounds[t][0][1] - box_bounds[t][0][0], box_bounds[t]
                            [1][1] - box_bounds[t][1][0], box_bounds[t][2][1] - box_bounds[t][2][0]])
...
def MSD_chain(r, ir, box_bounds, mol2ids):
    ...
    ccom = np.zeros([frames, mols, 3], float)
    # preallocate msd vectors
    msd_ccom = np.zeros(frames, float)
    msd_beads = np.zeros(frames, float)
    msd_ends = np.zeros(frames, float)
    msd_mid = np.zeros(frames, float)
    ...
    msd_beads = msd_beads / \
        np.arange(float(frames), 0, -1)/sum([len(mol) for mol in mol2ids])
...
def MSD_chain_xyz(r, ir, box_bounds, mol2ids):
    ...
    ccom = np.zeros([frames, mols, 3], float)
    # preallocate msd vectors
    msd_ccom = np.zeros([frames, 3], float)
    msd_beads = np.zeros([frames, 3], float)
    msd_ends = np.zeros([frames, 3], float)
    msd_mid = np.zeros([frames, 3], float)
    ...
    # scaling
    msd_ccom = np.transpose(np.transpose(msd_ccom) /
                            np.arange(float(frames), 0, -1))/mols
    msd_ends = np.transpose(np.transpose(msd_ends) /
                            np.arange(float(frames), 0, -1))/mols/2
    msd_mid = np.transpose(np.transpose(
        msd_mid)/np.arange(float(frames), 0, -1))/mols
    msd_beads = np.transpose(np.transpose(
        msd_beads)/np.arange(float(frames), 0, -1))/sum([len(mol) for mol in mol2ids])
...
def MSD_type(r, ir, box_bounds, id2type):
    ...
    msd = np.zeros([types+1, frames], float)
    ...
    for atom_type in range(1, types+1):
        msd[atom_type] = msd[atom_type] / \
            np.arange(float(frames), 0, -1)/count_types[atom_type]
...
def MSD_type_xyz(r, ir, box_bounds, id2type):
    ...
    # preallocate msd array, types+1 to start counting at 1
    msd = np.zeros([types+1, frames, 3], float)
    ...
    count_types = np.bincount(id2type)
    for atom_type in range(1, types+1):
        msd[atom_type] = np.transpose(np.transpose(
            msd[atom_type])/np.arange(float(frames), 0, -1))/count_types[atom_type]
